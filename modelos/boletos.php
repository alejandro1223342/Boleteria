...
public function insertar_boletos($eve_id,$cat_id_bol,$bol_precio,$bol_cantidad
								   ){
	$sql="CALL boletos_insert('$eve_id','$cat_id_bol','$bol_precio','$bol_cantidad');";
	return ejecutarConsultaSP($sql);
}
...
public function verificar($cli_cedula){
	...
	public function combo_tipoboletos($id){
	$sql="CALL sp_catalogo_select($id);";
	return ejecutarConsultaSP($sql);
}
	
	public function combo_eventos(){
	$sql="CALL sp_eventos_combo();";
	return ejecutarConsultaSP($sql);
}
